...
namespace GifComponents
{
    public class AnimatedGifEncoder
	{
		...
		protected GifImage image; // current frame
		 ...
		 */
		public bool AddFrame(GifImage im)
		{
		    if ((im == null) || !started) 
			{
				return false;
		 ...
		 *
		 */
		protected int FindClosest(GifColor c) 
		{
			if (colorTab == null) 
                return -1;
			...
			for (int i = 0; i < len;) 
			{
                int dr = c.R - (colorTab[i++] & 0xff);
                int dg = c.G - (colorTab[i++] & 0xff);
                int db = c.B - (colorTab[i] & 0xff);
	    ...
	    protected void GetImagePixels()
		{
	        ...
	        for (int i = image.Height - 1; i >= 0 ; i--)
	        {
	            for (int j = 0; j < image.Width; j++)
	            {
                    ...
                    pixels[count++] = image.PixelColors[index].R;
                    pixels[count++] = image.PixelColors[index].G;
                    pixels[count++] = image.PixelColors[index].B;
	            }
	        }
		}
    ...
    public class GifImage
    {
        public int Width;
        public int Height;
        public GifColor[] PixelColors;

        public GifImage(int w, int h, GifColor[] pixColors)
        {
            Width = w;
            Height = h;
            PixelColors = pixColors;
        }
    }
